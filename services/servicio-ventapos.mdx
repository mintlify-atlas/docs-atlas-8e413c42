...
description: "The servicio-ventapos orchestrates physical point-of-sale transactions, coordinating inventory, customers, and payment using the SAGA pattern for reliable rollback."
---
...
<Steps>
  ...
  <Step title="Check and Decrement Inventory">
    Call `servicio-inventario` at `POST /api/inventario/{productoId}/descontar?cantidad={n}` for each product in the sale. The inventory service atomically checks that sufficient stock exists and decrements it. If any product returns `400 Stock insuficiente`, the orchestrator triggers compensation for any items already decremented in this step and aborts.
  </Step>
  <Step title="Process Payment">
    Submit the total sale amount to the payment gateway. If payment is declined or the gateway returns an error, the orchestrator issues compensating inventory restoration calls (`POST /api/inventario/{productoId}/descontar` with a negative quantity, or a dedicated restore endpoint) to undo the stock decrements from Step 2, then returns a failure response to the POS terminal.
  </Step>
  <Step title="Confirm Sale">
    Once payment is successful, record the completed sale (line items, customer, timestamp, payment reference) in the service's own data store or forward the confirmation event to a downstream reporting service. Return a success response to the POS terminal with the sale receipt details.
  </Step>
</Steps>
...
All POS transaction requests from frontend clients must be routed through the API Gateway. The gateway's `ruta-orquestador-pos` route maps the external path prefix to this service.

| Context | Base Path |
|---|---|
| Via API Gateway (external) | `http://localhost:8080/api/v1/ventas-pos/**` |
| Direct to service (internal) | `http://localhost:8082/api/v1/ventas-pos/**` |

Example sale request through the gateway:

```bash
POST http://localhost:8080/api/v1/ventas-pos/procesar
Content-Type: application/json

{
  "clienteId": "CLI-001",
  "items": [
    { "productoId": "PROD-42", "cantidad": 2 },
    { "productoId": "PROD-17", "cantidad": 1 }
  ],
  "metodoPago": "TARJETA"
}
```
...
`servicio-ventapos` integrates with three external systems during a transaction. All calls are outbound HTTP — the POS service acts as client, not server, in these interactions.
