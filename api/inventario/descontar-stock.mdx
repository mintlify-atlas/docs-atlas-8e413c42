---
title: "POST /api/v1/inventario/{productoId}/descontar"
sidebarTitle: "POST Descontar"
description: "POST /api/v1/inventario/{productoId}/descontar — atomically decrements stock for a product by the given quantity. Returns 400 if stock is insufficient."
---
...
<ResponseField name="400 Bad Request" type="string">
  Returned when the stock is insufficient to fulfil the request, or when the `productoId` is not found in the database. Response body is the plain string:
...
**Failure — insufficient stock or product not found (400 Bad Request):**
...
- **Product not found:** If `inventarioRepository.findByProductoId(productoId)` returns an empty `Optional`, a `RuntimeException` is thrown (`"Producto no encontrado en inventario"`), which is caught by the controller logic and results in an HTTP 400 response.
